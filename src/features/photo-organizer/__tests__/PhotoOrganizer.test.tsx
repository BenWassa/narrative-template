...
vi.mock("../utils/coverStorageService", () => ({
  saveCover: vi.fn(
    async (projectId, blob) => `cover-${projectId}-${Date.now()}`
  ),
...
const samplePhotos = Array.from({ length: 6 }, (_, index) => {
  ...
  return {
    ...
    filePath:
      index < 4
        ? `Day 1/IMG_${1000 + index}.jpg`
        : `Day 2/IMG_${1000 + index}.jpg`,
  };
});
...
test.skip("renames a day label and export script uses it", async () => {
  ...
  fireEvent.click(first);
  const bucketBtn = await screen.findByRole("button", {
    name: /Establishing/i,
  });
...
test.skip("onboarding-selected day containers are shown even when empty", async () => {
  ...
  vi.mocked(projectService.getState).mockResolvedValue(
    stateWithSelectedContainer as any
  );
  render(<PhotoOrganizer />);
  const projectButton = await screen.findByRole("button", {
    name: /Test Trip/i,
...
test.skip("main menu shows app version badge and has no close button", async () => {
  ...
  expect(
    screen.queryByRole("button", { name: /Close welcome|Close/i })
  ).toBeNull();
});
...
test.skip("deleting a project removes recent entry and returns to welcome", async () => {
  ...
  const deleteMock = vi
    .spyOn(projectService as any, "deleteProject")
