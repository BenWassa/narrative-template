...
export function useProjectState({
  ...
  const [projectFolderLabel, setProjectFolderLabel] = useState<string | null>(
    null
  );
  const [projectSettings, setProjectSettings] =
    useState<ProjectSettings>(DEFAULT_SETTINGS);
  const [recentProjects, setRecentProjects] = useState<RecentProject[]>([]);
  const [showOnboarding, setShowOnboarding] = useState(false);
  const [showWelcome, setShowWelcome] = useState(
    () => !safeLocalStorage.get(ACTIVE_PROJECT_KEY)
  );
  const [projectError, setProjectError] = useState<string | null>(null);
  const [permissionRetryProjectId, setPermissionRetryProjectId] = useState<
    string | null
  >(null);
  const [projectNeedingReselection, setProjectNeedingReselection] = useState<
    string | null
  >(null);
  ...
  const updateRecentProjects = useCallback(
    (project: RecentProject) => {
      try {
        ...
        let existingIndex = normalized.findIndex(
          (p) => p.projectId === project.projectId
        );
        if (existingIndex === -1) {
          existingIndex = normalized.findIndex(
        ...
        if (err instanceof Error && err.name === "QuotaExceededError") {
          try {
            ...
            safeLocalStorage.set(
              RECENT_PROJECTS_KEY,
              JSON.stringify(withLimitedCovers)
            );
            setRecentProjects(withLimitedCovers);
            showToast(
              "Storage limit reached. Kept covers for 3 most recent projects.",
              "info"
            );
          } catch (retryErr) {
            try {
              ...
              showToast(
                "Storage critically low. Removed older project covers.",
                "error"
              );
            } catch (finalErr) {
              try {
                ...
                showToast(
                  "Storage full. Reset to current project only.",
                  "error"
                );
              } catch (clearErr) {
                ...
                setRecentProjects([]);
                showToast(
                  "Storage exhausted. Unable to save projects.",
                  "error"
                );
              }
            }
          ...
          showToast(
            "Failed to persist recent project updates. Changes may not be saved.",
            "error"
          );
        }
      }
  ...
  const applyFolderMappings = useCallback(
    (sourcePhotos: ProjectPhoto[], mappings: any[]) => {
  ...
  const applyDayContainers = useCallback(
    (sourcePhotos: ProjectPhoto[], containers: string[]) => {
  ...
  const loadProject = useCallback(
    async (projectId: string, options?: { addRecent?: boolean }) => {
      ...
      try {
        ...
        const photosWithDays = applyDayContainers(
          state.photos,
          state.dayContainers || []
        );
        ...
        if (options?.addRecent !== false) {
          ...
          let existingProject = normalized.find(
  ...
  const retryProjectPermission = useCallback(async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to retry permission";
      ...
      try {
        ...
        if (debugEnabled) {
          ...
          dayGroups.forEach((groupPhotos, day) => {
            ...
            console.group(
              `📆 Day ${day || "null"} (${groupPhotos.length} photos)`
            );
        ...
        const nextProjectName =
          state.projectName?.trim() || deriveProjectName(state.rootPath);
        ...
        if (reselectionProjectId) {
          ...
          const raw = safeLocalStorage.get(
            `${STATE_PREFIX}${reselectionProjectId}`
          );
          ...
          if (existingState.edits) {
            ...
            freshWithEdits = freshPhotos.map((photo) => {
              const cached = photo.filePath
                ? cachedEdits.get(photo.filePath)
                : null;
              if (cached) {
                return {
                  ...photo,
