...
describe("HEIC preview generation", () => {
  ...
  test("heicToBlob returns a blob when createImageBitmap succeeds", async () => {
    ...
    // @ts-ignore
    global.createImageBitmap = async (_file: File) => ({
      width: 16,
      height: 12,
    });
...
describe("Project file collection", () => {
  ...
  test("collects supported files, skips duplicates and system files", async () => {
    const img1 = new File(["aaa"], "IMG_0001.jpg", {
      type: "image/jpeg",
      lastModified: 1000,
    });
    const img1Dup = new File(["aaa"], "IMG_0001.jpg", {
      type: "image/jpeg",
      lastModified: 1000,
    });
    const img2 = new File(["bbbb"], "IMG_0002.jpg", {
      type: "image/jpeg",
      lastModified: 2000,
    });
    const rootImg = new File(["cc"], "ROOT_0001.jpg", {
      type: "image/jpeg",
      lastModified: 1500,
    });
    const dsStore = new File(["x"], ".DS_Store", {
    ...
    expect(names).toEqual(
      expect.arrayContaining(["IMG_0002.jpg", "ROOT_0001.jpg"])
    );
