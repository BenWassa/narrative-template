...
export async function buildPhotosFromHandle(
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (
      ...
      (pathAnalysis.confidence === "high" ||
        pathAnalysis.confidence === "medium")
    ) {
...
function isArchiveFolderSegment(segment: string, archiveFolder: string) {
  ...
  if (normalized.endsWith("_archive") || normalized.endsWith("-archive"))
    return true;
  if (normalized.includes("archive") || simplified.includes("archive"))
    return true;
  return false;
}

function applyArchiveFolder(
  photos: ProjectPhoto[],
  archiveFolder: string
): ProjectPhoto[] {
  ...
  return photos.map((photo) => {
    ...
    if (
      segments.some((segment) => isArchiveFolderSegment(segment, archiveFolder))
    ) {
...
export async function initProject(options: {
  ...
  if (!("requestPermission" in dirHandle)) {
    throw new Error(
      "File System Access API is not available or supported in this environment."
    );
  }
  ...
  if (permission !== "granted") {
    throw new Error(
      "Folder access was not granted. Please allow access to create the project."
    );
  }
...
export async function getState(projectId: string): Promise<ProjectState> {
  ...
  if (!handle) {
    throw new Error(
      "Project folder access not available. Please reselect the folder."
    );
  }
  ...
  if (permission !== "granted") {
    throw new Error(
      "Folder access was not granted. Please allow access to continue."
    );
  }
  ...
  if (freshPhotos.length === 0) {
    throw new Error(
      "Project folder access is no longer available. Please reselect the folder."
    );
  }
  ...
  const archivedPhotos = applyArchiveFolder(
    photos,
    settings.folderStructure?.archiveFolder ||
      DEFAULT_SETTINGS.folderStructure.archiveFolder
  );
