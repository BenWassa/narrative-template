...
export async function generateExportManifest(
  ...
  for (const photo of photos) {
    ...
    if (photo.archived) {
      ...
      const dayLabel =
        dayLabels[photo.day] || `Day ${String(photo.day).padStart(2, "0")}`;
...
export function generateUndoScript(manifest: ExportManifest): string {
  ...
  lines.push("");
  lines.push(
    `# Export timestamp: ${new Date(manifest.timestamp).toISOString()}`
  );
  ...
  if (manifest.operations.length > 10) {
    lines.push(
      `echo "  ... and ${manifest.operations.length - 10} more files"`
    );
  }
  lines.push("echo");
  ...
  manifest.operations.forEach((op) => {
    ...
    if (op.fileSize > 0) {
      ...
      lines.push(
        '    echo "${GREEN}✓ Deleted: ' + op.destinationPath + '${NC}"'
      );
      lines.push("  else");
      lines.push(
        '    echo "${RED}✗ Skipped (size mismatch): ' +
          op.destinationPath +
          '${NC}"'
      );
      lines.push("  fi");
    ...
    lines.push("else");
    lines.push(
      '  echo "${YELLOW}⚠ File not found: ' + op.destinationPath + '${NC}"'
    );
    lines.push("fi");
    lines.push("");
  ...
  manifest.operations.forEach((op) => {
    const dirPath = op.destinationPath.substring(
      0,
      op.destinationPath.lastIndexOf("/")
    );
...
function determineIfShouldExport(
  ...
  const hasUserAssignedDay =
    photo.day !== null && photo.day !== photo.detectedDay;
  const wasArchived =
    photo.archived && !photo.filePath?.includes(archiveFolder);
  ...
  if (photo.bucket && photo.day !== null) {
    ...
    needsToMove =
      photo.filePath !== targetPath && !photo.filePath?.includes(targetPath);
 ...
 */
export function saveExportManifest(
  projectId: string,
