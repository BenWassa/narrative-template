...
function detectCommonSourcePath(photos: ProjectPhoto[]): string {
  ...
  for (let i = 0; i < pathParts.length - 1; i++) {
    ...
    const allMatch = photosWithPaths.every((p) =>
  ...
  const commonPath = pathParts
    .slice(0, Math.max(commonDepth, pathParts.length - 2))
    .join("/");
  return commonPath || "/";
}
 ...
 */
function detectDaySourceFolder(
  dayPhotos: ProjectPhoto[],
  fallbackRoot: string
): string {
  if (dayPhotos.length === 0) {
    return fallbackRoot;
 ...
 */
export function autoDetectIngestState(
  photos: ProjectPhoto[],
  settings: any
): boolean {
